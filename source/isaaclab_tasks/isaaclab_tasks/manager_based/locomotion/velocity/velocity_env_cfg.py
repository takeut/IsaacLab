...
class ObservationsCfg:
    ...
    class PolicyCfg(ObsGroup):
        """Observations for policy group (Student - 実機で取得可能)."""
    ...
    class CriticCfg(ObsGroup):
        """Observations for critic group (Asymmetric Actor-Critic)."""
        
        # 全ての観測項目を含む（criticは学習時のみ使用）
        base_lin_vel = ObsTerm(func=mdp.base_lin_vel, noise=Unoise(n_min=-0.1, n_max=0.1))
        base_ang_vel = ObsTerm(func=mdp.base_ang_vel, noise=Unoise(n_min=-0.2, n_max=0.2))
        projected_gravity = ObsTerm(
            func=mdp.projected_gravity,
    ...
    # Teacher-Student構造用の観測グループ
    # observation groups
    policy: PolicyCfg = PolicyCfg()
    critic: CriticCfg = CriticCfg()  # Asymmetric Actor-Critic用
...
class EventCfg:
    ...
    randomize_actuator_gains = EventTerm(
        ...
        params={
            ...
            "stiffness_distribution_params": (15.0, 28.0),  # より広い範囲でKp（P制御のゲイン）をランダム化
            "damping_distribution_params": (0.07, 1.0),     # より広い範囲でKd（D制御のゲイン）をランダム化
            "operation": "abs",                          
